...
def train_DAE_model(net, task, data_loaders={},optimizer=None,loss_function=None,n_epochs=100,scheduler=None,load=False,save_path="model.pkl"):
    ...
    if not os.path.exists('save/' + task + '/AUCs'):
        os.makedirs('save/' + task + '/AUCs')
    file_AUCs = 'save/' + task + '/AUCs' + '/' + '_train_AUCs_' + '.txt'

    AUCs = ('Epoch\tLoss\tPhase')
    ...
    for epoch in range(n_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            train_AUCs = [epoch, epoch_loss, phase]
...
def train_CVAE_model(net,task,data_loaders={},optimizer=None,n_epochs=100,scheduler=None,load=False,save_path="model.pkl",best_model_cache = "drive"):
    ...
    if not os.path.exists('save/' + task + '_encoder/AUCs'):
        os.makedirs('save/' + task + '_encoder/AUCs')
    file_AUCs = 'save/' + task + '_encoder/AUCs' + '/' + '_train_AUCs_' + '.txt'
...
def train_predictor_model(net,data_loaders,optimizer,loss_function,n_epochs,scheduler,load=False,save_path="model.pkl"):
    ...
    if not os.path.exists('save/bulk_pre/AUCs'):
        os.makedirs('save/bulk_pre/AUCs')
    file_AUCs = 'save/bulk_pre/AUCs' + '/' + 'bulk_train_AUCs_' + '.txt'
...
def train_DaNN_model2(net,source_loader,target_loader,
    ...
    if not os.path.exists('save/DaNN/AUCs'):
        os.makedirs('save/DaNN/AUCs')
    file_AUCs = 'save/DaNN/AUCs' + '/' + '_train_AUCs_' + '.txt'
    AUCs = ('Epoch\tLoss\tMMD\tSCloss\tPhase')
    with open(file_AUCs, 'w') as f:
        f.write(AUCs + '\n')
    ...
    for epoch in range(n_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            train_AUCs = [epoch, epoch_loss, epoch_mmd, epoch_sc, phase]
            save_AUCs(train_AUCs, file_AUCs)
